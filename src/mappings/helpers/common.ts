...
import { fetchCrowdloan, getParachainId, parseBigInt, parseNumber } from "./utils";
...
export const ensureParachain = async (paraId: number, store : DatabaseManager): Promise<Parachain> => {
...
export const getLatestCrowdloanId = async (parachainId: string, store: DatabaseManager) => {
  const api = await apiService()
  ...
  const crowdloan =  new CrowdloanSequence({ id: parachainId, curIndex: 0, createdAt: new Date(), blockNum: curBlockNum.toNumber() })
